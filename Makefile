...
# Download integration tests (Go-based)
integrationtest-sda-download:
	@PR_NUMBER=$$(date +%F) docker compose -f .github/integration/sda-download-integration.yml up -d
	@sleep 10
	@PR_NUMBER=$$(date +%F) docker compose -f .github/integration/sda-download-integration.yml run --rm integration_test
	@PR_NUMBER=$$(date +%F) docker compose -f .github/integration/sda-download-integration.yml down -v --remove-orphans

integrationtest-sda-download-run:
	@PR_NUMBER=$$(date +%F) docker compose -f .github/integration/sda-download-integration.yml run --rm integration_test

integrationtest-sda-download-up: build-all
	@PR_NUMBER=$$(date +%F) docker compose -f .github/integration/sda-download-integration.yml up -d

integrationtest-sda-download-down:
	@PR_NUMBER=$$(date +%F) docker compose -f .github/integration/sda-download-integration.yml down -v --remove-orphans
