...
BEGIN
  IF (select max(version) from sda.dbschema_version) = sourcever then
    ...
    CREATE FUNCTION sda.register_file(submission_location TEXT, submission_file_path TEXT, submission_user TEXT)
    ...
    CREATE FUNCTION sda.set_archived(file_uuid UUID, archive_loc TEXT, file_path TEXT, file_size BIGINT, inbox_checksum_value TEXT, inbox_checksum_type TEXT)
