...
// StreamBodyOnly tests

func TestStreamBodyOnly_WholeFile(t *testing.T) {
	body := []byte("FULL_BODY_CONTENT_DATA")

	recorder := httptest.NewRecorder()

	err := StreamBodyOnly(StreamBodyConfig{
		Writer:          recorder,
		FileReader:      newReadSeekCloser(body),
		ArchiveFileSize: int64(len(body)),
		Range:           nil,
	...
	require.NoError(t, err)
	assert.Equal(t, fmt.Sprintf("%d", len(body)), recorder.Header().Get("Content-Length"))
	assert.Equal(t, "application/octet-stream", recorder.Header().Get("Content-Type"))
	assert.Equal(t, string(body), recorder.Body.String())
}

func TestStreamBodyOnly_RangeRequest(t *testing.T) {
	...
	err := StreamBodyOnly(StreamBodyConfig{
		...
		FileReader:      newReadSeekCloser(body),
		ArchiveFileSize: int64(len(body)),
		Range:           &RangeSpec{Start: 5, End: 14},
	})

	require.NoError(t, err)
	assert.Equal(t, 206, recorder.Code)
	assert.Equal(t, "bytes 5-14/20", recorder.Header().Get("Content-Range"))
	assert.Equal(t, "10", recorder.Header().Get("Content-Length"))
	assert.Equal(t, "FGHIJKLMNO", recorder.Body.String())
}

func TestStreamBodyOnly_RangeLastBytes(t *testing.T) {
	...
	err := StreamBodyOnly(StreamBodyConfig{
		...
		FileReader:      newReadSeekCloser(body),
		ArchiveFileSize: int64(len(body)),
		Range:           &RangeSpec{Start: 7, End: 9},
	})

	require.NoError(t, err)
	assert.Equal(t, 206, recorder.Code)
	assert.Equal(t, "HIJ", recorder.Body.String())
}

func TestStreamBodyOnly_NilFileReader(t *testing.T) {
	recorder := httptest.NewRecorder()
