...
func (sapit *SyncAPITest) SetupTest() {
...
func (sapit *SyncAPITest) TestSetup() {
	sapit.SetupTest()

	conf, err := config.NewConfig("sync-api")
	assert.NoError(sapit.T(), err, "Failed to setup config")
	assert.Equal(sapit.T(), mqPort, conf.Broker.Port)
	assert.Equal(sapit.T(), mqPort, viper.GetInt("broker.port"))

	server := setup(conf)
	assert.Equal(sapit.T(), "0.0.0.0:8080", server.Addr)
}

func (sapit *SyncAPITest) TestShutdown() {
	sapit.SetupTest()
	Conf, err = config.NewConfig("sync-api")
	assert.NoError(sapit.T(), err)

	Conf.API.MQ, err = broker.NewMQ(Conf.Broker)
	assert.NoError(sapit.T(), err)
	assert.Equal(sapit.T(), "127.0.0.1", Conf.API.MQ.Conf.Host)

	// make sure all conections are alive
	assert.Equal(sapit.T(), false, Conf.API.MQ.Channel.IsClosed())
	assert.Equal(sapit.T(), false, Conf.API.MQ.Connection.IsClosed())

	shutdown()
	assert.Equal(sapit.T(), true, Conf.API.MQ.Channel.IsClosed())
	assert.Equal(sapit.T(), true, Conf.API.MQ.Connection.IsClosed())
}

func (sapit *SyncAPITest) TestReadinessResponse() {
	sapit.SetupTest()
	Conf, err = config.NewConfig("sync-api")
	assert.NoError(sapit.T(), err)
	...
	assert.NoError(sapit.T(), err)
	assert.Equal(sapit.T(), http.StatusOK, res.StatusCode)
	...
	res, err = http.Get(ts.URL + "/ready")
	assert.NoError(sapit.T(), err)
	assert.Equal(sapit.T(), http.StatusServiceUnavailable, res.StatusCode)
	defer res.Body.Close()
...
func (sapit *SyncAPITest) TestDatasetRoute() {
	sapit.SetupTest()
	Conf, err = config.NewConfig("sync-api")
	assert.NoError(sapit.T(), err)
	...
	assert.NoError(sapit.T(), err)
	assert.Equal(sapit.T(), http.StatusOK, good.StatusCode)
	...
	assert.NoError(sapit.T(), err)
	assert.Equal(sapit.T(), http.StatusBadRequest, bad.StatusCode)
	defer bad.Body.Close()
}

func (sapit *SyncAPITest) TestMetadataRoute() {
	Conf = &config.Config{}
...
func (sapit *SyncAPITest) TestBasicAuth() {
	Conf = &config.Config{}
	Conf.Broker.SchemasPath = "../../schemas"
	...
	assert.NoError(sapit.T(), err)
	assert.Equal(sapit.T(), http.StatusUnauthorized, bad.StatusCode)
	defer bad.Body.Close()
}
