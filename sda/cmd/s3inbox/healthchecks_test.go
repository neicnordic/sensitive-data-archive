	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			newMocks: func() (*mocks.MockDatabase, *mocks.MockBroker, *mockServer) {
				...
				ms.On("ServeHTTP", http.MethodGet, "/ready", "", mock.Anything).Return(200, nil, nil).Once()
			...
			newMocks: func() (*mocks.MockDatabase, *mocks.MockBroker, *mockServer) {
				...
				mb.On("Alive").Return(true).Once()
				mdb.On("Ping").Return(nil).Once()
				ms.On("ServeHTTP", http.MethodGet, "/ready", "", mock.Anything).Return(500, nil, nil).Once()
			...
			newMocks: func() (*mocks.MockDatabase, *mocks.MockBroker, *mockServer) {
				...
				ms.On("ServeHTTP", http.MethodGet, "/ready", "", mock.Anything).Run(func(_ mock.Arguments) {
					time.Sleep(1 * time.Second)
