...
- `BROKER_SHUTDOWN_GRACE`: how long a message handler that is already running gets to finish after shutdown starts, before its context is cancelled, as a Go duration (default `20s`, `0` cancels it immediately). Keep it below the pod's `terminationGracePeriodSeconds`.
...
### Observability
The ingest service initializes the observability package, for configuration options and additional details see
