...
func (st *SyncTest) TestBuildSyncDatasetJSON() {
	st.SetupTest()
	...
	assert.NoError(st.T(), err, "failed to register file in database")
	...
	assert.NoError(st.T(), err, "failed to mark file as Archived")
	err = db.SetVerified(fileInfo, fileID, corrID)
	assert.NoError(st.T(), err, "failed to mark file as Verified")

	accessions := []string{"ed6af454-d910-49e3-8cda-488a6f246e67"}
	assert.NoError(st.T(), db.MapFilesToDataset("cd532362-e06e-4461-8490-b9ce64b8d9e7", accessions), "failed to map file to dataset")
	...
	assert.Equal(st.T(), string(dataset), string(jsonData))
}

func (st *SyncTest) TestCreateHostURL() {
	conf = &config.Config{}
	...
	assert.NoError(st.T(), err)
	assert.Equal(st.T(), "http://localhost:443/dataset", s)
}

func (st *SyncTest) TestSendPOST() {
	r := http.NewServeMux()
	...
	assert.EqualError(st.T(), sendPOST(syncJSON), "401 Unauthorized")
}
