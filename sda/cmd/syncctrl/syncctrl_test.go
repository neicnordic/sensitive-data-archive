...
func (suite *SyncTest) SetupSuite() {
	...
	// _, err = mq.Channel.QueueDeclare("mappings", false, true, false, false, nil)
	// assert.NoError(suite.T(), err)
	...
	assert.NoError(suite.T(), db.MapFilesToDataset("aa-Dataset-cd532362", accessions), "failed to map file to dataset")

	msg := []byte(`{"type":"mapping", "dataset_id": "aa-Dataset-cd532362", "accession_ids": ["accession-1","accession-2","accession-3"]}`)
...
func (suite *SyncTest) TestHandleDatasetMsg() {
	...
	msg := []byte(`{"type":"release", "dataset_id": "aa-Dataset-cd532362"}`)
	assert.NoError(suite.T(), mq.SendMessage("", "", "mappings", msg), "failed to send message")
}
