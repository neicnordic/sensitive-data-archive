...
func (ts *DatabaseTests) TestUpdateFileEventLog() {
	...
	err = ts.verificationDB.QueryRow("SELECT last_event FROM sda.files WHERE id=$1", fileID).Scan(&lastEvent)
	assert.NoError(ts.T(), err, "Failed to read denormalized file event")
	assert.Equal(ts.T(), "uploaded", lastEvent, "UpdateFileEventLog() did not update files.last_event")
}

func (ts *DatabaseTests) TestUpdateFileEventLogRollbackRestoresLastEvent() {
	fileID, err := ts.db.RegisterFile(context.Background(), nil, "/inbox", "/testuser/file4-rollback.c4gh", "testuser")
	assert.NoError(ts.T(), err, "failed to register file in database")

	var lastEventBefore string
	err = ts.verificationDB.QueryRow("SELECT last_event FROM sda.files WHERE id=$1", fileID).Scan(&lastEventBefore)
	assert.NoError(ts.T(), err, "failed to read last_event before transaction")
	assert.Equal(ts.T(), "registered", lastEventBefore)

	tx, err := ts.verificationDB.Begin()
	assert.NoError(ts.T(), err, "failed to begin transaction")

	_, err = tx.Exec("INSERT INTO sda.file_event_log (file_id, event, user_id, details, message) VALUES ($1, 'disabled', 'system', '{}', '{}')", fileID)
	assert.NoError(ts.T(), err, "failed to insert file event inside transaction")

	var lastEventInTx string
	err = tx.QueryRow("SELECT last_event FROM sda.files WHERE id=$1", fileID).Scan(&lastEventInTx)
	assert.NoError(ts.T(), err, "failed to read last_event inside transaction")
	assert.Equal(ts.T(), "disabled", lastEventInTx)

	err = tx.Rollback()
	assert.NoError(ts.T(), err, "failed to rollback transaction")

	var lastEventAfter string
	err = ts.verificationDB.QueryRow("SELECT last_event FROM sda.files WHERE id=$1", fileID).Scan(&lastEventAfter)
	assert.NoError(ts.T(), err, "failed to read last_event after rollback")
	assert.Equal(ts.T(), "registered", lastEventAfter)
}
...
func (ts *DatabaseTests) TestGetUserFiles() {
	...
	disabledFileID, err := ts.db.RegisterFile(context.Background(), nil, "/inbox", fmt.Sprintf("%v/submission_b/TestGetUserFiles-disabled.c4gh", testUser), testUser)
	...
	assert.Equal(ts.T(), "", nextCursor)

	filelist, _, err = ts.db.GetUserFiles(context.Background(), testUser, "", true, 0, "")
	...
	page, cursor, err := ts.db.GetUserFiles(context.Background(), testUser, "", true, pageLimit, "")
	assert.NoError(ts.T(), err, "failed to get first paginated page")
	assert.LessOrEqual(ts.T(), len(page), pageLimit, "first page exceeds limit")
	assert.NotEmpty(ts.T(), cursor, "expected non-empty cursor when more files remain")
	for _, fi := range page {
		seen[fi.FileID] = struct{}{}
	}

	for cursor != "" {
		page, cursor, err = ts.db.GetUserFiles(context.Background(), testUser, "", true, pageLimit, cursor)
		assert.NoError(ts.T(), err, "failed to get subsequent paginated page")
		assert.LessOrEqual(ts.T(), len(page), pageLimit, "page exceeds limit")
		for _, fi := range page {
			_, dup := seen[fi.FileID]
			assert.False(ts.T(), dup, "duplicate file ID across pages: %s", fi.FileID)
			seen[fi.FileID] = struct{}{}
		}
	}

	assert.Equal(ts.T(), testCases, len(seen), "pagination did not return all files without duplicates")

	// An invalid cursor must return ErrInvalidCursor.
	_, _, err = ts.db.GetUserFiles(context.Background(), testUser, "", true, pageLimit, "not-a-valid-cursor!!!")
	assert.ErrorIs(ts.T(), err, database.ErrInvalidCursor, "expected ErrInvalidCursor for bad cursor")
	...
	_, _, err = ts.db.GetUserFiles(context.Background(), testUser, "", true, pageLimit, "Zm9v")
