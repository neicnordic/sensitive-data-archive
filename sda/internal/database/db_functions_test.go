...
func (suite *DatabaseTests) TestRegisterFile() {
	...
	for _, step := range []struct {
		...
		suite.T().Run(step.name, func(t *testing.T) {
			fileID, err := db.RegisterFile(&step.id, step.location, step.filePath, step.userName)
...
func (suite *DatabaseTests) TestGetArchived() {
	...
	err = db.SetArchived("/archvie", fileInfo, fileID)
...
func (suite *DatabaseTests) TestGetCorrID_sameFilePath() {
	...
	if err := db.SetArchived("/archvie", fileInfo, fileID); err != nil {
