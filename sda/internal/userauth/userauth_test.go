...
func (s *UserAuthTest) TestAlwaysAuthenticator() {
...
func (s *UserAuthTest) TestUserTokenAuthenticator_NoFile() {
	a := NewValidateFromToken(jwk.NewSet())
	err := a.ReadJwtPubKeyPath("")
	assert.Error(s.T(), err)
}

func (s *UserAuthTest) TestUserTokenAuthenticator_GetFile() {
	...
	assert.NoError(s.T(), err)
	assert.Equal(s.T(), 2, a.Keyset.Len())

	defer os.RemoveAll(demoKeysPath)
}

func (s *UserAuthTest) TestUserTokenAuthenticator_WrongURL() {
	a := NewValidateFromToken(jwk.NewSet())
	jwtpubkeyurl := "/dummy/"

	err := a.FetchJwtPubKeyURL(jwtpubkeyurl)
	assert.Equal(s.T(), "jwtpubkeyurl is not a proper URL (/dummy/)", err.Error())
}

func (s *UserAuthTest) TestUserTokenAuthenticator_BadURL() {
	a := NewValidateFromToken(jwk.NewSet())
	jwtpubkeyurl := "dummy.com/jwk"

	err := a.FetchJwtPubKeyURL(jwtpubkeyurl)
	assert.Equal(s.T(), "parse \"dummy.com/jwk\": invalid URI for request", err.Error())
}

func (s *UserAuthTest) TestUserTokenAuthenticator_GoodURL() {
	a := NewValidateFromToken(jwk.NewSet())
	jwtpubkeyurl := fmt.Sprintf("http://localhost:%d/jwk", OIDCport)

	err := a.FetchJwtPubKeyURL(jwtpubkeyurl)
	assert.NoError(s.T(), err, "failed to fetch remote JWK")
	assert.Equal(s.T(), 3, a.Keyset.Len())
}

func (s *UserAuthTest) TestUserTokenAuthenticator_ValidateSignature_RSA() {
	...
	assert.Equal(s.T(), privateClaims["pilot"], helper.DefaultTokenClaims["pilot"])
	...
	assert.Equal(s.T(), "\"iat\" not satisfied", nonvalidToken.Error())
	...
	assert.Contains(s.T(), brokenToken.Error(), "failed to parse jws: failed to parse JOSE headers:")
	...
	assert.Contains(s.T(), brokenToken2.Error(), "failed to parse jws: failed to parse JOSE headers:")
	...
	r.URL.Path = "/dummy/"
	_, err = a.Authenticate(r)
	assert.Contains(s.T(), err.Error(), "failed to get issuer from token")

	// Delete the keys when testing is done or failed
	defer os.RemoveAll(demoKeysPath)
}

func (s *UserAuthTest) TestUserTokenAuthenticator_ValidateSignature_EC() {
...
func (s *UserAuthTest) TestWrongKeyType_RSA() {
	// Create temp demo ec key pair
	demoKeysPath := "demo-ec-keys"
	demoPrKeyName := "/ec"
	prKeyPath, pubKeyPath, err := helper.MakeFolder(demoKeysPath)
	...
	assert.Equal(s.T(), "bad key format, expected RSA got EC", err.Error())

	defer os.RemoveAll(demoKeysPath)
}

func (s *UserAuthTest) TestWrongKeyType_EC() {
	// Create temp demo ec key pair
	demoKeysPath := "demo-rsa-keys"
	demoPrKeyName := "/rsa"
	prKeyPath, pubKeyPath, err := helper.MakeFolder(demoKeysPath)
	...
	assert.Equal(s.T(), "bad key format, expected EC got RSA", err.Error())

	defer os.RemoveAll(demoKeysPath)
}

func (s *UserAuthTest) TestUserTokenAuthenticator_ValidateSignature_HS() {
	...
	assert.Contains(s.T(), WrongAlg.Error(), "failed to find key with key ID")
}
