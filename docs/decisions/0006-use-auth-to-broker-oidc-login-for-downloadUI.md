...
### Summary of changes (implementated)
...
2. Direct OIDC in the webapp (bypass `sda-auth`)
   ...
   - would need configured OIDC service at the IdP

## Decision Outcome

See proposed solution and justification above.

### Consequences

#### Positive

- Avoids re-implementing an OIDC client for a webapp that is part of the sda stack.
...
- In-memory store requires sticky sessions or single replica.
...
## How to test proposed implementation (local)
