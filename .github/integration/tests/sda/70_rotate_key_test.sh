...
for file in testfile1 testfile2; do
    if [ ! -f "$file" ]; then
...
if [ "$response" -ne 2 ]; then
	echo "files for rotatekey test failed to upload"
	exit 1
fi
...
curl -s -k -H "Authorization: Bearer $token" -H "Content-Type: application/json" -X POST -d '{"filepath": "dataset_rotatekey/testfile2.c4gh", "user": "test@dummy.org"}' http://api:8080/file/ingest
checkStatus verified 2
...
curl -s -k -H "Authorization: Bearer $token" -H "Content-Type: application/json" -X POST -d '{"accession_id": "ROTATE-KEY-02", "filepath": "dataset_rotatekey/testfile2.c4gh", "user": "test@dummy.org"}' http://api:8080/file/accession
checkStatus ready 2

curl -s -k -H "Authorization: Bearer $token" -H "Content-Type: application/json" -X POST -d '{"accession_ids": ["ROTATE-KEY-01", "ROTATE-KEY-02"], "dataset_id": "KEY-ROTATION-TEST-0001", "user": "test@dummy.org"}' http://api:8080/dataset/create
checkStatus ready 0
...
if "$(psql -U postgres -h postgres -d sda -At -c "select key_hash from sda.files where stable_id like 'ROTATE-KEY-0%';" | grep -c "$rotatekeyHash")" -neq 2;
then
...
# download files with rotated key, concatenate header and archive body, decrypt and chack
