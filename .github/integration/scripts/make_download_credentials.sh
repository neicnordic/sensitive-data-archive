#!/bin/sh
# Simplified credentials script for download service integration tests
# This version doesn't require RabbitMQ
set -e
...
# Create database credentials for download service
for n in api download; do
...
# create RSA key for signing the JWT tokens
...
echo "creating token"
python /scripts/sign_jwt.py integration_test@example.org > "/shared/token"

# Create trusted issuers file for visa validation
