...
for n in download finalize inbox ingest mapper sync verify; do
    ...
    for DB in postgres remote-postgres; do
        psql -U postgres -h "$DB" -d sda -c "ALTER ROLE $n LOGIN PASSWORD '$n';"
        psql -U postgres -h "$DB" -d sda -c "GRANT base TO $n;"
    done
    ## password and permissions for MQ

    for HOST in rabbitmq remote-rabbitmq; do
        body_data=$(jq -n -c --arg password "$n" --arg tags none '$ARGS.named')
        curl -s -u guest:guest -X PUT -k "http://$HOST:15672/api/users/$n" -H "content-type:application/json" -d "${body_data}"
        curl -s -u guest:guest -X PUT -k "http://$HOST:15672/api/permissions/sda/$n" -H "content-type:application/json" -d '{"configure":"","write":"sda","read":".*"}'
    done
done
