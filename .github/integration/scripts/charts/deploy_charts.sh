...
if [ "$1" == "sda-mq" ]; then
    ...
    if [ "$4" == "federated" ]; then
      if [ "$3" = true ] ; then
        kubectl exec broker-sda-mq-0 -- rabbitmqadmin --username=admin --port=15671 --use-tls --tls-cert-file=/etc/rabbitmq/tls/tls.crt --tls-key-file=/etc/rabbitmq/tls/tls.key --tls-ca-cert-file=/etc/rabbitmq/tls/ca.crt --password="$ADMINPASS" declare queue --vhost=sda name=from_cega durable=true
      else
