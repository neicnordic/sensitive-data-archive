...
Edit the values.yaml file and specify the relevant parts of the `global` section. If no shared credentials for the
broker and database are used, the credentials for each service shuld be set in the
`credentials` section.

While it is possible to deploy this chart with the crypt4gh keys included in the values file as base64 encoded strings,
it is advisable to create the secret containing the crypt4gh keys manually.

## Upgrading an existing Release to a new version

A major chart version change (like v1.2.3 -> v2.0.0) indicates that there is an incompatible breaking change needing
manual actions.

### To 4.0.0

This version replaces flat scalar storage keys (`s3Url`, `s3BucketPrefix`,
`volumePath`, `storageType`, ...) with nested `s3:` and `posix:` lists on
`global.archive`, `global.backupArchive`, `global.inbox`, and
`global.sync.destination`, enabling multi-endpoint storage.

Existing 3.x values files will not render in 4.0 without conversion. See [MIGRATION-4.0.md](./MIGRATION-4.0.md) for the
upgrade procedure, the volume-name rename table (`archive` → `archive-0`, `backup` →
`backup-archive-0`, `inbox` → `inbox-0`), the DOA fail-fast caveat, and multi-endpoint examples.

### Storage configuration

Each backend (`global.archive`, `global.backupArchive`, `global.inbox`,
`global.sync.destination`) accepts a list of endpoints under `s3:`
and/or `posix:`. At least one list must be non-empty for backends that need storage.

Minimal single-endpoint S3 archive:
...
See [MIGRATION-4.0.md](./MIGRATION-4.0.md) for upgrades from 3.x and multi-endpoint examples (sharded, mixed s3+posix,
multiple NFS).

#### Storage endpoint fields

Each entry under `s3:` accepts:

| Field            | Description                                                          | Default     |
|------------------|----------------------------------------------------------------------|-------------|
| `endpoint`       | URL of the S3 endpoint, e.g. `https://s3.example.com`. **Required.** | —           |
| `port`           | Optional explicit port; usually inferred from the URL scheme.        | —           |
| `bucketPrefix`   | Prefix for generated buckets (`${BUCKET_PREFIX}${1-MAX_BUCKETS}`).   | per backend |
| `region`         | S3 region.                                                           | `us-east-1` |
| `accessKey`      | Access key. **Required.**                                            | —           |
| `secretKey`      | Secret key. **Required.**                                            | —           |
| `chunkSize`      | S3 chunk size (`5mb`–`1gb`, e.g. `15mb`).                            | app default |
| `maxBuckets`     | Number of buckets to auto-create per endpoint.                       | `1`         |
| `maxObjects`     | Object-count rollover threshold (`0` = unlimited).                   | `0`         |
| `maxSize`        | Byte-size rollover threshold (e.g. `5tb`, `500mb`).                  | unlimited   |
| `writerDisabled` | Mark this endpoint read-only (excluded from writer rotation).        | `false`     |

Each entry under `posix:` accepts:

| Field                  | Description                                         | Default   |
|------------------------|-----------------------------------------------------|-----------|
| `path`                 | Filesystem path inside the container. **Required.** | —         |
| `maxObjects`           | Object-count rollover threshold (`0` = unlimited).  | `0`       |
| `maxSize`              | Byte-size rollover threshold.                       | unlimited |
| `writerDisabled`       | Mark this endpoint read-only.                       | `false`   |
| `volume.existingClaim` | Mount an existing PVC.                              | —         |
| `volume.nfsServer`     | Mount via NFS (host or IP).                         | —         |
| `volume.nfsPath`       | NFS export path (required with `nfsServer`).        | —         |

Endpoints are rendered as a list under `storage.<backend>.s3` /
`storage.<backend>.posix` in the consuming service config. Volumes for POSIX endpoints are named `<backend>-<index>`
(e.g. `archive-0`); the DOA legacy service reads only `archive.posix[0]` and keeps the un-suffixed `archive` volume
name.
...
Unless the same queries are being executed manually by a database administrator a `Basic authentication Secret`
containing the credentials to perform the upgrade needs to be created.
...
| `global.schemaType`                              | Standalone version requires a custom JSON schema, available options are `federated` or `isolated`, unset defaults to federated.                                                                          | `""`                                            |
| `global.ingress.deploy`                          |                                                                                                                                                                                                          | `false`                                         |
| `global.ingress.hostName.auth`                   |                                                                                                                                                                                                          | `""`                                            |
| `global.ingress.hostName.doa`                    |                                                                                                                                                                                                          | `""`                                            |
| `global.ingress.hostName.s3Inbox`                |                                                                                                                                                                                                          | `""`                                            |
| `global.ingress.secretNames.auth`                | The name of a manually created secret holding the certificates for the ingress enpoint.                                                                                                                  | `""`                                            |
| `global.ingress.secretNames.doa`                 | The name of a manually created secret holding the certificates for the ingress enpoint.                                                                                                                  | `""`                                            |
| `global.ingress.secretNames.s3Inbox`             | The name of a manually created secret holding the certificates for the ingress enpoint.                                                                                                                  | `""`                                            |
...
| `global.archive.s3`                              | List of S3 archive endpoints (chart 4.0+). At least one of `s3` or `posix` must be non-empty. Endpoint fields are described under "Storage endpoint fields" below.                                       | `[]`                                            |
| `global.archive.posix`                           | List of POSIX archive endpoints (chart 4.0+). At least one of `s3` or `posix` must be non-empty. Endpoint fields are described under "Storage endpoint fields" below.                                    | `[]`                                            |
| `global.archive.locationBrokerCacheTTL`          | Sets the TTL for the object count and size cache used for the archive storage. Expects a duration string (eg: `60s`, `5m`). If unset, the application default is used.                                   | `""`                                            |
| `global.backupArchive.s3`                        | List of S3 backup-archive endpoints (chart 4.0+). Leave both `s3` and `posix` empty to disable backup. Endpoint fields are described under "Storage endpoint fields" below.                              | `[]`                                            |
| `global.backupArchive.posix`                     | List of POSIX backup-archive endpoints (chart 4.0+). Leave both `s3` and `posix` empty to disable backup. Endpoint fields are described under "Storage endpoint fields" below.                           | `[]`                                            |
| `global.backupArchive.locationBrokerCacheTTL`    | Sets the TTL for the object count and size cache used for the backup storage. Expects a duration string (eg: `60s`, `5m`). If unset, the application default is used.                                    | `""`                                            |
...
| `global.broker.rotateKeyQueue`                   | MQ queue name for key rotation messages.                                                                                                                                                                 | `"rotatekey"`                                   |
...
| `global.c4gh.rotatePubKeyData`                   | The target public key for rotation (base64 encoded string).                                                                                                                                              | `""`                                            |
...
| `global.db.admin.username`                       | If secretName is not set will default to this to populate PGUSER used in migration jobs                                                                                                                  | `""`                                            |
| `global.db.admin.password`                       | If secretName is not set will default to this to populate PGPASSWORD used in migration jobs                                                                                                              | `""`                                            |
...
| `global.db.maxIdleConnections`                   | Sets the maximum number of connections in the idle connection pool, if set to <= 0 no idle connections are retained.                                                                                     | `2`                                             |
| `global.db.maxOpenConnections`                   | Sets the maximum number of open connections to the database, set to <= 0 for unlimited                                                                                                                   | `20`                                            |
| `global.db.connectionMaxIdleTime`                | Sets the maximum amount of time a connection may be idle, set to <= 0 for unlimited. Expects a go time.Duration parsable string                                                                          | `"5m"`                                          |
| `global.db.connectionMaxLifeTime`                | Sets the maximum amount of time a connection may be reused, set to <= 0 for unlimited. Expects a go time.Duration parsable string                                                                        | `"0"`                                           |
...
| `global.oidc.acrValues`                          | Space separated authentication context class references, one of which the login must have been made with, e.g. `https://refeds.org/profile/mfa` to enforce two factor authentication.                    | ``                                              |
| `global.inbox.servicePort`                       | The port that the inbox is accessible via.                                                                                                                                                               | `2222`                                          |
| `global.inbox.s3`                                | List of S3 inbox endpoints used by pipeline readers (chart 4.0+). The sftp-inbox writer uses `posix[0]`; the s3-inbox writer is configured via `global.s3Inbox` below.                                   | `[]`                                            |
| `global.inbox.posix`                             | List of POSIX inbox endpoints (chart 4.0+). The sftp-inbox writer mounts and writes to entry `[0]`; pipeline readers can consume all entries. Endpoint fields below.                                     | `[]`                                            |
| `global.inbox.locationBrokerCacheTTL`            | Sets the TTL for the object count and size cache used for the inbox storage. Expects a duration string (eg: `60s`, `5m`). If unset, the application default is used.                                     | `""`                                            |
| `global.s3Inbox.url`                             | URL to S3 inbox instance  (if port is included in url s3Port should be set as 0).                                                                                                                        | `""`                                            |
| `global.s3Inbox.port`                            | Port that the S3 inbox is available on.                                                                                                                                                                  | ``                                              |
| `global.s3Inbox.bucket`                          | S3 archive bucket prefix, buckets will be created with the following naming convention ${BUCKET_PREFIX}${1-MAX_BUCKETS}                                                                                  | `"inbox"`                                       |
| `global.s3Inbox.accessKey`                       | Access key to S3 inbox .                                                                                                                                                                                 | `null`                                          |
| `global.s3Inbox.secretKey`                       | Secret key to S3 inbox.                                                                                                                                                                                  | `null`                                          |
| `global.s3Inbox.region`                          | S3 inbox region.                                                                                                                                                                                         | `"us-east-1"`                                   |
| `global.s3Inbox.readyPath`                       | Path to the s3 ready/healthy endpoint.                                                                                                                                                                   | `null`                                          |
| `global.sync.enabled`                            | If the sync service is to be enabled and deployed                                                                                                                                                        | `null`                                          |
...
| `global.sync.centerPrefix`                       | Only sync datasets which has this prefix in the accession id, if not set all datasets will be synced                                                                                                     | `null`                                          |
| `global.sync.destination.s3`                     | List of S3 sync destination endpoints (chart 4.0+). At least one of `s3` or `posix` must be non-empty when sync is enabled. Endpoint fields below.                                                       | `[]`                                            |
| `global.sync.destination.posix`                  | List of POSIX sync destination endpoints (chart 4.0+). At least one of `s3` or `posix` must be non-empty when sync is enabled. Endpoint fields below.                                                    | `[]`                                            |
| `global.sync.destination.locationBrokerCacheTTL` | Sets the TTL for the object count and size cache used for the sync storage. Expects a duration string (eg: `60s`, `5m`). If unset, the application default is used.                                      | `""`                                            |
| `global.sync.remote.url`                         | The URL to an http endpoint which is to get notifications about datasets being synced, if not set no http sync notifications will be sent. Example: https://synt.to.here:1234/dataset_sync               | `null`                                          |
| `global.sync.remote.password`                    | Password to be used for HTTP basic auth on requests to remote.url, if not set basic auth will not be used when sending http notifications.                                                               | `null`                                          |
| `global.sync.remote.user`                        | Username to be used for HTTP basic auth on requests to remote.url, if not set basic auth will not be used when sending http notifications.                                                               | `null`                                          |
...
---------------------------------------|----------------------------------------------------------------------|------------------------------------
 ...
 `download.minClientVersion`           | Minimal version of the `sda-cli` to accept                           | `v0.3.0`                           
 ...
 `rotatekey.replicaCount`              | Desired number of replicas.                                          | `1`                                
 `rotatekey.annotations`               | Specific annotations for the rotatekey pod.                          | `{}`                               
 `rotatekey.resources.requests.memory` | Memory request for container.                                        | `128Mi`                            
 `rotatekey.resources.requests.cpu`    | CPU request for container.                                           | `100m`                             
 `rotatekey.resources.limits.memory`   | Memory limit for container.                                          | `256Mi`                            
 `rotatekey.resources.limits.cpu`      | CPU limit for container.                                             | `1000m`                            
