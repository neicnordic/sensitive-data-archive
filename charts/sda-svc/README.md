...
This version adds a Job that sets the first registered crypt4gh key to all ingested files that do not have a c4gh key referenced.
Unless the same query is being exiecuted manually by a database adminitstrator a `Basic authentication Secret` containg the credentials to perform the upgrade needs to be created.
