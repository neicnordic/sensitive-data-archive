...
If no shared credentials for the broker and database are used, the credentials for each service shuld be set in the
`credentials` section.
...
Existing 3.x values files will not render in 4.0 without conversion.
See [MIGRATION-4.0.md](./MIGRATION-4.0.md) for the upgrade procedure,
the volume-name rename table (`archive` → `archive-0`, `backup` →
`backup-archive-0`, `inbox` → `inbox-0`), the DOA fail-fast caveat, and
...
and/or `posix:`. At least one list must be non-empty for backends that
need storage.
...
See [MIGRATION-4.0.md](./MIGRATION-4.0.md) for upgrades from 3.x and
multi-endpoint examples (sharded, mixed s3+posix, multiple NFS).
...
`storage.<backend>.posix` in the consuming service config. Volumes for
POSIX endpoints are named `<backend>-<index>` (e.g. `archive-0`); the
DOA legacy service reads only `archive.posix[0]` and keeps the
un-suffixed `archive` volume name.
...
| `global.reencrypt.timeout`                       | Timeout in seconds for calls to the reencryption gRPC service                                                                                                                                                                                                                                  | `30`                                              |
